...
class _RemedyState extends State<Remedy> {
  ...
  return Scaffold(
    appBar: AppBar(title: Text('Remedies',style: TextStyle(fontSize: 24),),backgroundColor: Colors.amber,),
    body: Center(
      child:Column(
          ...
          children: <Widget>[
            ...
            for(var item in widget.disease ) quanti[0]!=0?Text("${y+1}. ${rem_dis[item['label']][0]}: ${(((rem_dis[item['label']][1])*quanti[y++])).toInt()}ml per can \n",
              style: TextStyle(fontSize:20,color: Colors.white,backgroundColor: Colors.amber ),)
                                                           :Text(" "),
            SizedBox(
              height: MediaQuery.of(context).size.height*.06,
            ),
            Text("NOTE : Here can = 20 lit......"),
          ]
      ),
    ),
  );
